<template>
  <div class="products-page">
    ...
    <section class="hero-section">
      <div class="hero-bg">
        <span class="hero-label">{{ $t('newArrivals.freshDrops') }}</span>
        <h1 class="hero-title">{{ $t('newArrivals.newArrivals') }}</h1>
        <p v-if="meta" class="hero-count">{{ $t('products.count', { count: meta.total }) }}</p>
      </div>
    </section>
      ...
      <div class="toolbar">
        <div class="toolbar-left">
          <div class="search-wrapper">
            ...
            <input
              id="newarrivals-search"
              name="search"
              v-model="filters.search"
              type="text"
              :placeholder="$t('search.placeholderNewArrivals')"
              class="search-input"
            />
          </div>
        </div>
            ...
            <div class="mobile-filter-group">
              ...
              <Transition name="filter-collapse">
                <div v-if="filterOpen.price" class="mobile-filter-options">
                  <input id="na-price-mobile" name="price" type="range" :min="0" :max="maxPrice" :value="currentPrice" step="5" @input="onPriceInput" class="price-range" />
      ...
      <div class="layout">
            ...
            <Transition name="filter-collapse">
              <div v-if="filterOpen.gender" class="filter-body">
                <div v-for="g in filterOptions.genders" :key="g" class="filter-checkbox" @click="toggleFilter('gender', g)">
                  <input :id="'na-g-'+g" name="gender" type="checkbox" :checked="filters.gender?.includes(g)" class="checkbox-input" />
            ...
            <Transition name="filter-collapse">
              <div v-if="filterOpen.brand" class="filter-body">
                <div v-for="b in filterOptions.brands" :key="b" class="filter-checkbox" @click="toggleFilter('brand', b)">
                  <input :id="'na-b-'+b" name="brand" type="checkbox" :checked="filters.brand?.includes(b)" class="checkbox-input" />
            ...
            <Transition name="filter-collapse">
              <div v-if="filterOpen.type" class="filter-body">
                <div v-for="t in filterOptions.types" :key="t" class="filter-checkbox" @click="toggleFilter('type', t)">
                  <input :id="'na-t-'+t" name="type" type="checkbox" :checked="filters.type?.includes(t)" class="checkbox-input" />
            ...
            <Transition name="filter-collapse">
              <div v-if="filterOpen.department" class="filter-body">
                <div v-for="d in filterOptions.departments" :key="d" class="filter-checkbox" @click="toggleFilter('department', d)">
                  <input :id="'na-d-'+d" name="department" type="checkbox" :checked="filters.department?.includes(d)" class="checkbox-input" />
          ...
          <div class="card filter-card">
            ...
            <Transition name="filter-collapse">
              <div v-if="filterOpen.price" class="filter-body">
                <input id="na-price-desktop" name="price" type="range" :min="0" :max="maxPrice" :value="currentPrice" step="5" @input="onPriceInput" class="price-range" />
                <div class="price-labels">
                  <span>$0</span>
                  <span>up to ${{ currentPrice }}</span>
                </div>
        ...
        <div class="main-content">
          ...
          <div v-else-if="error" class="state-card">
            ...
            <p class="state-text">{{ $t('newArrivals.errorLoading') }}</p>
            <button @click="loadProducts" class="btn-primary">{{ $t('products.tryAgain') }}</button>
          </div>

          <div v-else-if="!products.length" class="state-card">
            <i class="ti ti-package-off state-icon" aria-hidden="true" />
            <p class="state-text">{{ $t('newArrivals.emptyFilters') }}</p>
...
} = useProductFilters({ defaultSort: 'newest' })
</script>
